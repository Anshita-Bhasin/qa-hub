      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-slate-500">Headless Playwright Engine</span>
          </div>
        ...
        <div className="flex items-center space-x-2">
          <div className="text-right hidden sm:block">
            <span className="text-[10px] text-slate-500 block">SQLite Storage:</span>
            <span className="text-xs text-emerald-700 font-medium">table: scraper_runs</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-semibold text-slate-900">Target Configuration & Scan Scope</span>
          </div>
          <span className="text-xs text-slate-500">Target: https://www.saucedemo.com</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center">
          ...
          <div className="lg:col-span-4 space-y-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-white border border-slate-300 rounded-lg px-3.5 py-2.5 text-xs text-slate-700 focus:outline-none focus:border-slate-400 focus:ring-1 focus:ring-slate-400"
              >
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
            <button
              ...
              className="flex items-center space-x-2 text-slate-700 hover:text-slate-900 text-left"
            >
              {check404 ? <CheckSquare className="w-4 h-4 text-slate-700" /> : <Square className="w-4 h-4 text-slate-500" />}
            ...
            <button
              ...
              className="flex items-center space-x-2 text-slate-700 hover:text-slate-900 text-left"
            >
              {detectZeroPrices ? <CheckSquare className="w-4 h-4 text-slate-700" /> : <Square className="w-4 h-4 text-slate-500" />}
            ...
            <button
              ...
              className="flex items-center space-x-2 text-slate-700 hover:text-slate-900 text-left"
            >
              {duplicateHash ? <CheckSquare className="w-4 h-4 text-slate-700" /> : <Square className="w-4 h-4 text-slate-500" />}
            ...
            <button
              ...
              className="flex items-center space-x-2 text-slate-700 hover:text-slate-900 text-left"
            >
              {deepLinkVerify ? <CheckSquare className="w-4 h-4 text-slate-700" /> : <Square className="w-4 h-4 text-slate-500" />}
          ...
          <div className="lg:col-span-3 flex flex-col justify-center">
            <button
              ...
              className={`w-full flex items-center justify-center space-x-2 py-3 px-4 rounded-lg font-medium text-xs transition-all shadow-md ${
                isScraping
                  ? 'bg-slate-900/60 text-slate-300 cursor-not-allowed'
                  : 'bg-[#FFD21E] hover:bg-[#FFC107] text-slate-900'
              }`}
            >
            ...
            {isScraping && (
              <div className="w-full bg-slate-100 rounded-full h-1.5 mt-2 overflow-hidden">
                <div
                  className="bg-[#FFD21E] h-1.5 rounded-full transition-all duration-300"
        ...
        <div className="pt-3 border-t border-slate-200 flex flex-wrap items-center justify-between text-xs text-slate-500">
          <div className="flex items-center space-x-2">
            ...
            <span>
              Stored in local.db → table: <strong className="text-slate-700">scraper_runs</strong> (Last run: {lastScrapeInfo.time}, duration: {lastScrapeInfo.duration})
            </span>
          </div>
          <div className="flex items-center space-x-3 mt-1 sm:mt-0">
            ...
            {lastScrapeInfo.anomaliesFound > 0 && (
              <button
                ...
                className="flex items-center space-x-1 text-xs text-slate-700 hover:text-slate-900 font-medium underline"
              >
                {syncedToLog ? (
                  <>
                    <Check className="w-3.5 h-3.5 text-emerald-700" />
                    <span className="text-emerald-700">Synced to Issues Log!</span>
                  </>
                ) : (
                  <>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-900">
              Live Audit Results Matrix: <span className="text-slate-700">{selectedPersona}</span>
            </h2>
            <p className="text-xs text-slate-500">
              Scraped product entities with asset verification, extracted prices, and deep links
            </p>
          </div>
          <span className="text-xs text-slate-500 px-2.5 py-1 rounded bg-slate-100 border border-slate-200">
            6 Items Scraped
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayedProducts.map((prod) => (
            <div
              key={prod.id}
              className={`bg-white border rounded-xl overflow-hidden shadow-sm transition-all flex flex-col justify-between ${
            ...
            >
              <div className="p-4 space-y-3">
                {/* Image & Status Tag */}
                <div className="relative rounded-lg overflow-hidden bg-white border border-slate-200 aspect-video flex items-center justify-center p-2">
                  <img
                    src={prod.imgUrl}
                    alt={prod.name}
                  ...
                  {prod.isDuplicateAsset && (
                    <div className="absolute bottom-2 left-2 right-2">
                      <span className="px-2 py-0.5 rounded text-[10px] bg-slate-900/40 text-amber-700 border border-amber-300 block truncate">
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm font-semibold text-slate-900 line-clamp-1">{prod.name}</h3>
                  ...
                  <div className="flex items-center space-x-2 text-[11px] text-slate-500 mt-1">
                    ...
                    <span>•</span>
                    <span className="text-slate-500">{prod.location}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-4 py-2.5 bg-white border-t border-slate-200 flex items-center justify-between text-xs">
                <span className="text-slate-500">Deep link verification</span>
                <a
                  href={prod.deepLink}
                  target="_blank"
                  rel="noreferrer"
